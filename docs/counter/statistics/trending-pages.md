...
```
  ...
  - If `ignoreNewPages` (Exclude pages without previous visits) is set to true, pages without visits in the previous date range are not shown. The default value is false. 
